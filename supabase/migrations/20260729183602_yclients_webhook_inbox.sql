create unique index crm_connections_yclients_salon_id_uidx
  on public.crm_connections ((configuration ->> 'salon_id'))
  where provider = 'yclients'::public.crm_provider
    and configuration ? 'salon_id';

create table private.yclients_webhook_inbox (
  ...
  constraint yclients_webhook_inbox_provider
    check (provider::text = 'yclients'),
  constraint yclients_webhook_inbox_company_id
    check (company_id > 0),
  constraint yclients_webhook_inbox_resource
    check (
      char_length(resource) between 1 and 64
      and resource ~ '^[a-z][a-z0-9_]{0,63}$'
    ),
  constraint yclients_webhook_inbox_resource_id
    check (resource_id > 0),
  constraint yclients_webhook_inbox_event_status
    check (
      char_length(event_status) between 1 and 32
      and event_status ~ '^[a-z][a-z0-9_-]{0,31}$'
    ),
  constraint yclients_webhook_inbox_processing_status
    check (
  ...
  constraint yclients_webhook_inbox_raw_payload_object
    check (jsonb_typeof(raw_payload) = 'object'),
  constraint yclients_webhook_inbox_raw_payload_size
    check (octet_length(raw_payload::text) <= 262144),
  constraint yclients_webhook_inbox_payload_hash_size
    check (octet_length(payload_hash) = 32),
  constraint yclients_webhook_inbox_error_code
    check (
      error_code is null
      or (
  ...
  constraint yclients_webhook_inbox_processing_result
    check (
      (
  ...
  constraint yclients_webhook_inbox_pending_has_no_error
    check (processing_status <> 'pending' or error_code is null),
  constraint yclients_webhook_inbox_payload_hash_unique
    unique (provider, payload_hash)
);

create index yclients_webhook_inbox_organization_idx
  on private.yclients_webhook_inbox (organization_id);

create index yclients_webhook_inbox_connection_idx
  on private.yclients_webhook_inbox (connection_id);

create index yclients_webhook_inbox_pending_received_idx
  on private.yclients_webhook_inbox (received_at)
  where processing_status = 'pending';

alter table private.yclients_webhook_inbox enable row level security;
alter table private.yclients_webhook_inbox force row level security;

revoke all
on private.yclients_webhook_inbox
from public, anon, authenticated, service_role;

create function private.store_yclients_webhook_event_internal(
...
begin
  ...
  where connection.provider = 'yclients'::public.crm_provider
  ...
  if target_connection_status = 'disconnected'::public.crm_connection_status
  then
    return query select 'connection_unavailable'::text, null::uuid;
    return;
  end if;
  ...
  insert into private.yclients_webhook_inbox (
    provider,
    organization_id,
    connection_id,
    company_id,
  ...
  if inserted_event_id is null then
    ...
    from private.yclients_webhook_inbox as inbox
    where inbox.provider::text = 'yclients'
...
revoke all
on function private.store_yclients_webhook_event_internal(
  bigint,
  text,
  bigint,
  text,
...
create function public.store_yclients_webhook_event(
  p_company_id bigint,
  p_resource text,
...
as $$
  select *
  from private.store_yclients_webhook_event_internal(
    p_company_id,
    p_resource,
    p_resource_id,
    p_event_status,
...
comment on table private.yclients_webhook_inbox is
  'Append-only server webhook inbox. Browser roles have no privileges or RLS policies.';

comment on column private.yclients_webhook_inbox.raw_payload is
  'Validated, size-bounded YCLIENTS JSON. It may contain customer data and must never be exposed or logged.';

comment on function private.store_yclients_webhook_event_internal(
  bigint,
  text,
  bigint,
  text,
...
is 'SECURITY DEFINER is limited to service_role and validates the complete payload before resolving a non-disconnected YCLIENTS connection and appending an event.';
